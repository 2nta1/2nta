...
export default function PostComposer({ onPostCreated }: PostComposerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log('composer session', session);
      ...
      const res = await fetch('/api/user/posts', {
        ...
        body: JSON.stringify({ content, image: base64Image, userId: session?.user?.id }),
      });
      const responseBody = await res.json();
      if (!res.ok) {
        console.error('Error creating post', res.status, responseBody);
      }
      created = res.ok ? responseBody : undefined;
    } catch (err) {
      console.error(err);
    }
