...
import { FaBuilding, FaEnvelope, FaLock, FaUserTie, FaPhone, FaGlobe, FaArrowLeft } from 'react-icons/fa'
...
export default function CompanyRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.message || 'حدث خطأ في تسجيل الشركة')
      }

      router.push('/auth/company/login')
    } catch (err: any) {
      setError(err.message || 'حدث خطأ في إنشاء حساب الشركة')
    } finally {
      setIsLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-4xl">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          قم بإنشاء حساب شركتك للوصول إلى لوحة التحكم
        </p>
      </div>
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-4xl">
        <div className="bg-white py-8 px-6 shadow-xl rounded-2xl sm:px-10 border border-gray-100">
          ...
          <div className="mt-6 text-center text-sm">
            ...
            >
              لديك حساب شركة بالفعل؟ سجل الدخول
            </Link>
          </div>
        </div>
      </div>
